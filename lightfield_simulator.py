...
class OBJECT_OT_update_focalstack(bpy.types.Operator):
    """Update the focal stack setup"""
    bl_idname = "scene.update_focalstack"
    bl_label = """Update the focal stack setup"""
    bl_options = {'REGISTER'}
    ...
    def execute(self, context):
        ...
        try:
            # save current position and orientation of the camera grid
            lightfield = bpy.data.objects[LF.get_focalstack_name()]
            LF.center_cam_x, LF.center_cam_y, LF.center_cam_z = lightfield.location
            LF.center_cam_rot_x, LF.center_cam_rot_y, LF.center_cam_rot_z = lightfield.rotation_euler
        ...
        bpy.ops.scene.create_fstack('EXEC_DEFAULT')

        return {'FINISHED'}


class OBJECT_OT_create_fstack(bpy.types.Operator):
    """Create the light field setup"""
    bl_idname = "scene.create_fstack"
    ...
    def execute(self, context):
        ...
        try:
            ...
            camera = LF.get_focalstack_cameras()
            camera.select = True
            ...
            lightfield.name = LF.get_focalstack_name()
    ...
    def create_cameras(self):
        ...
        for i in range(1):
            for j in range(1):
                cameras.append(self.create_camera(LF.get_focalstack_camera_name(400,400), pos_x, pos_y, pos_z, 0, 0))
        return cameras
    ...
    def create_camera(self, cam_name, x_pos, y_pos, z_pos, theta, phi, eta=0):
        ...
        camera.data.lens = LF.focal_length_fs
        camera.data.sensor_width = LF.sensor_size_fs
        camera.data.sensor_height = LF.sensor_size_fs
    ...
    def set_render_properties(self):
        ...
        scene.render.resolution_x = LF.x_res_fs
        scene.render.resolution_y = LF.y_res_fs
        scene.render.resolution_percentage = 100

class OBJECT_OT_delete_focalstack(bpy.types.Operator):
    """Delete lightfield"""
    bl_idname = "scene.delete_focalstack"
    bl_label = """Delete the focal stack setup"""
    bl_options = {'REGISTER'}
...
class OBJECT_OT_render_focalstack(bpy.types.Operator):
    """render focal stack"""
    bl_idname = "scene.render_focalstack"
    bl_label = """Render Focal stack"""
    bl_options = {'REGISTER'}
    ...
    def get_focus_coordinates(self,disp):
        LF = bpy.context.scene.LF
        max_res = max(LF.x_res, LF.y_res)

        if LF.focus_dist == 0:
            factor = LF.baseline_x_m * LF.focal_length * max_res
            G_plus = factor / (disp * LF.sensor_size)

        else:
            factor = LF.baseline_x_m * LF.focal_length * LF.focus_dist * max_res
            G_plus = factor / (LF.baseline_x_m * LF.focal_length * max_res +
                               disp * LF.focus_dist * LF.sensor_size)

	#LF.focus_dist_fs = G_plus 
    ...
    def render_input_views(self, cameras, scene_key, LF, tgt_dir):
        ...
        # render view per focus point
        c_image = 'Image'


        dof_range = np.linspace(LF.frustum_min_disp ,LF.frustum_max_disp , LF.fstack_num)

        for i , value in enumerate(dof_range): 

            value = round(value, 2)

            focus_point = self.get_focus_coordinates(value)
	   
            print("Rendering scene with focus distance: " + str(value) )
            image_filename = 'input_FS_' + str(i)
            image_out_node.file_slots[c_image].path = image_filename + '_frame###'
            c_image = image_filename + '_frame###'

            # set scene camera to current light field camera
            bpy.data.scenes[scene_key].camera = cameras

            bpy.data.scenes[scene_key].camera.data.dof_distance = focus_point
            
            # change seed
            bpy.data.scenes[scene_key].cycles.seed = LF.cycles_seed + i
            #print("Cycles seed for camera %d: %d" % (cam_idx, bpy.data.scenes[scene_key].cycles.seed))
	...
	#rendering all-in-focus image 
        print("Rendering scene all-in-focus: " + str(0) )
        image_filename = 'input_FS_' + 'all_in_focus'
        image_out_node.file_slots[c_image].path = image_filename + '_frame###'
        c_image = image_filename + '_frame###'
        ...
        bpy.data.scenes[scene_key].camera.data.cycles.aperture_fstop = LF.fstop
    ...
    def renderFrame(self, tgt_dir = None):
        ...
        try:
            for node in bpy.context.scene.node_tree.nodes:
                if node.name.startswith("FS"):
                    bpy.context.scene.node_tree.nodes.remove(node)
        except KeyError:
            pass

        lf_cameras = LF.get_focalstack_cameras()
        LF.cycles_seed = random.randint(0, 2147483646  - 1)
        ...
        # save parameters as config file in target directory of rendering
        tmp_config_path = LF.path_config_file
        LF.path_config_file = os.path.join(tgt_dir, 'parameters_fs.cfg')
        bpy.ops.scene.save_focalstack('EXEC_DEFAULT')
        LF.path_config_file = tmp_config_path
