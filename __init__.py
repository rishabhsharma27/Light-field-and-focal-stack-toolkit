...
# focal stack camera parameters
    focal_length_fs = FloatProperty(
        name='f-Len[mm]',
        default=100,
        min=0,
        max=1000,
        description='Focal length of cameras [mm]',
        update=updates.update_focalstack
    )
    fstack_num = IntProperty(
        name='Number of images',
        default=21,
        min=1,
        max=100,
        description='No. of images in focal stack',
        update=updates.update_focalstack
    )
    x_res_fs = IntProperty(
        name='xRes[px]',
        default=512,
        min=1,
        max=10000,
    ...
    def get_focalstack_cameras(self):
        camera_name = self.get_focalstack_camera_name(400, 400)
        try:
    ...
    def get_focalstack_camera_name(self,i,j):
        return "FS%s_Cam%3.3i" % (self.setup_number, i*self.num_cams_x+j)

    def get_lightfield_name(self):
        return "LF%s" % self.setup_number

    def get_focalstack_name(self):
        return "FS%s" % self.setup_number
