...
class OBJECT_OT_save_focalstack(bpy.types.Operator):
    """Save config file with focal stackcamera setup"""
    bl_idname = "scene.save_focalstack"
    bl_label = """Save focal stack parameters"""
    bl_options = {'REGISTER'}
    ...
    def execute(self, context):
        ...
        parser.set(section, 'focal_length_mm', str(LF.focal_length_fs))
        parser.set(section, 'image_resolution_x_px', str(LF.x_res_fs))
        parser.set(section, 'image_resolution_y_px', str(LF.y_res_fs))
        parser.set(section, 'sensor_size_mm', str(LF.sensor_size_fs))
        parser.set(section, 'fstop', str(LF.fstop_fs))
        parser.set(section, 'number of focal stack images', str(LF.fstack_num))
        ...
        a = np.linspace(LF.frustum_min_disp,LF.frustum_max_disp,LF.fstack_num)

        parser.set(section, 'disparity values', str(a))
